export const localStorageController = {
  ...
  setProjectsList: (projectsList) => localStorage.setItem(
    'OnTimed: projects-list',
  ...
  getAllTasks: () => {
    ...
    const arrayOfProjectIds = projectsList.map(({ id, name, iconURL }) => ({
      id,
    }));

    const allTasksList = arrayOfProjectIds.flatMap(({ id }) => Object.values(localStorageController.getTasksListByProjectId(id)));
    return allTasksList;
  },
