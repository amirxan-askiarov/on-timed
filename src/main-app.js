...
import {
  ...
  tasksPageController,
} from './pages-navs/pages-controller';
...
class Application {
  ...
  start() {
    ...
    projectsPageController
      .getPageItems(DEFAULT_PAGE, projectsList)
      .forEach((project) => renderProject(project));

    renderProjectsPageNav(
      DEFAULT_PAGE,
      projectsPageController.pagesTotal(projectsList),
    );
    ...
    renderTasksPageNav(
      DEFAULT_PAGE,
      tasksPageController.pagesTotal(defaultFilteredSortedGroup),
    );

    const defaultTasksFirstPage = tasksPageController.getPageItems(
      DEFAULT_PAGE,
      defaultFilteredSortedGroup,
    );
  ...
  createNewProject(inputNewProject, currentProjectsPage) {
    ...
    const currentPageLength = projectsPageController.getPageItems(
      currentProjectsPage,
      projectsList,
    ).length;

    const projectId = localStorageController.getProjectId();
    const newProjectsList = projectsController.createNew(
      projectId,
      projectsList,
      inputNewProject,
    );
    ...
    renderProjectsPageNav(
      currentProjectsPage,
      projectsPageController.pagesTotal(newProjectsList),
    );
  ...
  editProject(inputEditedProject) {
    ...
    const currentTasksList = localStorageController.getTasksListByProjectId(
      editedProject.id,
    );
    const editedTasksList = tasksController.updateProjectName(
      currentTasksList,
      editedProject.name,
    );

    localStorageController.setTasksListByProjectId(
      editedProject.id,
      editedTasksList,
    );
  ...
  removeProject(
    ...
    const { editedProjectsList, removedId } = projectsController.remove(
      projectsList,
      projectId,
    );
    ...
    const newFilteredSortedGroup = this.applyViewOptions(
      viewState,
      this.getTasksGroup(groupId),
    );

    const newProjectsPageView = projectsPageController.getPageItems(
      newProjectsPageNumber,
      editedProjectsList,
    );
    const newTasksPageView = tasksPageController.getPageItems(
      DEFAULT_PAGE,
      newFilteredSortedGroup,
    );
    ...
    renderTasksPageNav(
      DEFAULT_PAGE,
      tasksPageController.pagesTotal(newFilteredSortedGroup),
    );
  ...
  createNewTask(inputNewTask, currentTasksPage) {
    ...
    const currentPageLength = tasksPageController.getPageItems(
      currentTasksPage,
      currentTasksList,
    ).length;

    const taskId = localStorageController.getTaskId();
    const result = tasksController.createNew(
      taskId,
      currentTasksList,
      projectName,
      inputNewTask,
    ...
    renderTasksPageNav(
      currentTasksPage,
      tasksPageController.pagesTotal(newTasksList),
    );
  ...
  toggleTaskStatus(projectId, taskId) {
    const currentTasksList = localStorageController.getTasksListByProjectId(projectId);
    const { editedTasksList, editedStatus } = tasksController.toggleTaskStatus(
      currentTasksList,
      taskId,
    );
  ...
  removeTask(projectId, taskId, currentTasksPage, currentTasksPageLength) {
    ...
    renderTasksPageNav(
      newTasksPageNumber,
      tasksPageController.pagesTotal(editedTaskList),
    );
  ...
  getTasksGroup(newGroupIdentifier) {
    ...
    if (Object.values(STANDARD_GROUPS).includes(newGroupIdentifier)) {
      ...
      newGroup = groupsController.getTaskListByGroup(
        allTasks,
        newGroupIdentifier,
      );
    } else {
  ...
  applyViewOptions(viewState, tasksGroup, currentTasksPage) {
    ...
    if (!group) {
      group = this.getTasksGroup(
        localStorageController.getCurrentGroupIdentifier(),
      );
    }
    ...
    let tasksPage;
    if (!currentTasksPage) {
      tasksPage = tasksPageController.getPageItems(
        DEFAULT_PAGE,
        filteredSortedTasks,
      );
      renderTasksPageNav(
        DEFAULT_PAGE,
        tasksPageController.pagesTotal(filteredSortedTasks),
      );
      return tasksPage;
    }
  ...
  moveProjectsPageForward(projectsPageNumber) {
    const { projectsList } = localStorageController.getProjectsList();
    const { newPageNumber, newPage } = projectsPageController.movePageForward(
      projectsPageNumber,
      projectsList,
    );
    renderProjectsPageNav(
  ...
  moveProjectsPageBackwards(projectsPageNumber) {
    const { projectsList } = localStorageController.getProjectsList();
    const { newPageNumber, newPage } = projectsPageController.movePageBackwards(
      projectsPageNumber,
      projectsList,
    );
    renderProjectsPageNav(
  ...
  moveTasksPageForward(tasksPageNumber) {
    const currentFilteredSortedGroup = this.applyViewOptions(
      this.getViewState(),
      null,
      tasksPageNumber,
    );
    const { newPageNumber, newPage } = tasksPageController.movePageForward(
      tasksPageNumber,
      currentFilteredSortedGroup,
    );
    renderTasksPageNav(
      newPageNumber,
      tasksPageController.pagesTotal(currentFilteredSortedGroup),
    );
    return { newPage, newPageNumber };
  }
  ...
  moveTasksPageBackwards(tasksPageNumber) {
    ...
    const { newPageNumber, newPage } = tasksPageController.movePageBackwards(
      tasksPageNumber,
      currentFilteredSortedGroup,
    );
    renderTasksPageNav(
