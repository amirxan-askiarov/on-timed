...
export const tasksController = {
  createNew: (taskId, tasksList, projectName, inputNewTask) => {
    ...
    if (!Array.isArray(tasksList)) {
      throw createErrorObj(ERR_CONTROLLER.TASK_LIST);
    }
    if (
      !projectName
      || !eachIsValid(projectId)
      || !title
      || !dueDate
      || !priority
      ...
      throw createErrorObj(ERR_CONTROLLER.TASK_DATA);
    }
  ...
  edit: (tasksList, inputEditedTask) => {
    ...
    if (!eachIsValid(editedTaskIndex)) {
      throw createErrorObj(ERR_CONTROLLER.NO_INDEX);
    }
    ...
    if (editedTask.status !== '0') {
      editedTask.status = updateOverdueStatus(
        editedTask.status,
        Date.parse(editedTask.dueDate),
      );
    }
  ...
  updateProjectName: (tasksList, editedProjectName) => {
    ...
    if (!eachIsValid(editedProjectName)) {
      throw createErrorObj(ERR_CONTROLLER.TASK_DATA);
    }
  ...
  remove: (tasksList, taskId) => {
    ...
    if (!eachIsValid(removedTaskIndex)) {
      throw createErrorObj(ERR_CONTROLLER.NO_INDEX);
    }
