import { NUM_PROJECTS_PAGE, NUM_TASKS_PAGE, createErrorObj } from '../utils';
import { ERR_CONTROLLER } from './errors-text';

const pagesController = (numTasksPerPage) => ({
  movePageForward: (currentPage, listArr) => {
    if (typeof currentPage !== 'number') {
      throw createErrorObj(ERR_CONTROLLER.NUM);
    }
    if (!Array.isArray(listArr)) {
      throw createErrorObj(ERR_CONTROLLER.LIST);
    }
    ...
    const newPage = pagesController(numTasksPerPage).getPageItems(
      newPageNumber,
      listArr,
    );
