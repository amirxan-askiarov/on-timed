...
export function initializeFocusModules() {
  ...
  const removeMenuBtnCancel = document.querySelector(
    ".remove-menu .button-box .exit",
  );
  const errorModal = document.querySelector(".error-modal");
  const errorModalBtnOk = document.querySelector(
  ...
  if (
    ...
    const err = createErrorObj([
      "Application error",
      "One or more of the menus components couldn't be found",
      "Process: applying the observers on the menus",
    ]);
    showErrorModal(err);
    return;
  }
  ...
  projectMenuFocusApplier.observe(projectMenu, {
    subtree: true,
    childList: true,
  });
  taskMenuFocusApplier.observe(taskMenu, { subtree: true, childList: true });
  removeMenuFocusApplier.observe(removeMenu, {
    subtree: true,
    childList: true,
  });
  errorModalFocusApplier.observe(errorModal, {
    subtree: true,
    childList: true,
  });
