...
export function createErrorObj([title, message, process]) {
...
export function safeThrottle(fn, onErrorFn, delay) {
...
export function showErrorModal(errorObj) {
  if (!isObject(errorObj)) {
    showErrorModal([
      'Application error',
      "Heading and/or error message couldn't be found",
      'Process: rendering error modal',
    ]);
    return;
  }
  ...
  if (eachIsValid(errorObj.title)) {
    messageHeading.textContent = errorObj.title;
  } else {
  ...
  messagePara.textContent = errorObj.message;
  process.textContent = errorObj.process;
}
...
export function handleExitRemoveMenu(e) {
  if (isPressedKey(e)) {
    ...
    if (
      !eachIsHTMLElement(menuCover)
      || !eachIsHTMLElement(removeMenu)
      || !eachIsHTMLElement(heading)
      || !eachIsHTMLElement(message)
    ) {
      showErrorModal([
        'Application error',
        "One or more menu components couldn't be found",
        'Process: exiting the remove confirmation menu',
      ]);
      return;
    }
...
export function eachIsHTMLElement(...elements) {
  return elements.every(
    (el) => el instanceof HTMLElement || el instanceof Element,
  );
}

export function eachIsObject(...objs) {
  return objs.every((obj) => obj instanceof Object);
}

export function eachIsBoolean(...values) {
  return values.every((value) => typeof value === 'boolean');
}

export function eachExists(...values) {
  return values.every((value) => value !== undefined && value !== null);
}

export function eachIsValid(...values) {
  return values.every(
  ...
  return new Proxy(baseEnum, {
    get(target, name) {
      ...
      if (!baseEnum.hasOwnProperty(name)) {
        throw createErrorObj(['Application error', `"${name}" value does not exist in the enum`, '']);
      }
      return baseEnum[name];
    },
    set(target, name, value) {
      throw createErrorObj(['Application error', 'Cannot add a new value to the enum', '']);
    },
  });
}
