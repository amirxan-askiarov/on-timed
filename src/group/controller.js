import { STATUS, STANDARD_GROUPS, createErrorObj } from "../utils";
...
export const groupsController = {
  getTaskListByGroup: (taskList, groupIdentifier) => {
    ...
    switch (groupIdentifier) {
      ...
      default:
        throw createErrorObj(ERR_CONTROLLER.DEFAULT_ID);
    }
  },
};

const filterTasksByToday = (taskList) =>
  taskList.filter(({ dueDate }) => {
    const parsedDueDate = new Date(dueDate);
    const now = new Date();

    return (
      now.getDate() === parsedDueDate.getDate() &&
...
const filterTasksByStatus = (taskList, status) =>
  taskList.filter(({ status: taskStatus }) => taskStatus === status);
