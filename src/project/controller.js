...
export const projectsController = {
  createNew: (projectId, projectsList, inputNewProject) => {
    ...
    if (!Array.isArray(projectsList)) {
      throw createErrorObj(ERR_CONTROLLER.PROJECT_LIST);
    }
    if (!name || !iconURL || !altText) {
      throw createErrorObj(ERR_CONTROLLER.PROJECT_DATA);
    }
  ...
  edit: (projectsList, inputEditedProject) => {
    ...
    if (!eachIsValid(id)) {
      throw createErrorObj(ERR_CONTROLLER.INVALID_INDEX);
    }
    if (!name || !iconURL || !altText) {
      throw createErrorObj(ERR_CONTROLLER.PROJECT_DATA);
    }
    ...
    if (!eachIsValid(storedProjectIndex)) {
  ...
  remove: (projectsList, projectId) => {
    ...
    if (!eachIsValid(projectId)) {
      throw createErrorObj(ERR_CONTROLLER.INVALID_INDEX);
    }

    const removedProjectIndex = findIndex(projectsList, projectId);
    if (!eachIsValid(removedProjectIndex)) {
      throw createErrorObj(ERR_CONTROLLER.NO_INDEX);
    }
